...
function useData(user, localMode=false){
  const [fazendas,setFazendas]=useState([]), [equipamentos,setEquipamentos]=useState([]), [visitas,setVisitas]=useState([]), [checklists,setChecklists]=useState([]), [diagnosticos,setDiagnosticos]=useState([]), [planejamentos,setPlanejamentos]=useState([]), [obstaculos,setObstaculos]=useState([]), [testesCobertura,setTestesCobertura]=useState([]);
  ...
  return {
    ...
    userId:user?.id, fazendas, equipamentos, visitas, checklists, diagnosticos, planejamentos, obstaculos, testesCobertura,
    ...
    savePlanejamento: r => upsert('planejamentos_antena', setPlanejamentos, 'cta_planejamentos', {...r, user_id:user?.id}),
    saveObstaculo: r => upsert('obstaculos_cobertura', setObstaculos, 'cta_obstaculos', {...r, user_id:user?.id}),
    saveTesteCobertura: r => upsert('testes_cobertura', setTestesCobertura, 'cta_testes_cobertura', {...r, user_id:user?.id}),
...
function App(){
  ...
  return <div className="app"><NotificationCenter/><Sidebar view={view} setView={setMainView} user={user} cloud={data.cloud} localMode={localMode} onExitLocal={()=>{localStorage.removeItem(LOCAL_MODE_KEY); setLocalMode(false)}}/><main className="main">
...
function Fazendas({data,onOpen}){
  ...
  const farms=data.fazendas.filter(f=>{
    const equipmentText=data.equipamentos.filter(e=>e.fazenda_id===f.id).map(e=>`${e.tipo} ${e.codigo_original} ${e.apelido}`).join(' ');
    ...
    const matchesQuick=quick==='todos'||(quick==='sem-gps'&&!(f.latitude&&f.longitude))||(quick==='sem-equip'&&!data.equipamentos.some(e=>e.fazenda_id===f.id))||(quick==='pendencias'&&(f.status==='Com pendência'||data.visitas.some(v=>v.fazenda_id===f.id&&v.pendencias)));
    return matchesText&&matchesCentral&&matchesStatus&&matchesQuick;
  });
  const counts={alta:data.fazendas.filter(f=>f.central==='Alta Genetics').length,genex:data.fazendas.filter(f=>f.central==='Genex Brasil').length,andamento:data.fazendas.filter(f=>f.status==='Em andamento').length,pendencias:data.fazendas.filter(f=>f.status==='Com pendência').length,finalizadas:data.fazendas.filter(f=>['Finalizada','Instalação concluída'].includes(f.status)).length};
  return <div><PageHead eyebrow="Operação de campo" title="Visão geral das fazendas" desc="Encontre rapidamente fazendas, equipamentos, status e responsáveis."><button className="btn primary" onClick={()=>setModal(true)}><Plus size={18}/> Nova fazenda</button></PageHead>
    ...
    <BrasilAtuacaoMap fazendas={data.fazendas} onOpen={onOpen}/>
    <section className="panel smartFilters"><div className="filterHeader"><div><h2><Filter size={20}/> Filtros rápidos</h2><p>Combine filtros para localizar exatamente o que precisa.</p></div><button className="btn light" onClick={()=>{setQ('');setCentral('Todas');setStatus('Todos');setQuick('todos')}}>Limpar filtros</button></div><div className="toolbar"><div className="search"><Search size={18}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar fazenda, cidade, regional, veterinário, equipamento ou código..."/></div><select value={central} onChange={e=>setCentral(e.target.value)}><option>Todas</option>{CENTRAIS.map(c=><option key={c}>{c}</option>)}</select><select value={status} onChange={e=>setStatus(e.target.value)}><option>Todos</option>{FARM_STATUS.map(s=><option key={s}>{s}</option>)}</select></div><div className="filterChips"><button className={quick==='todos'?'active':''} onClick={()=>setQuick('todos')}>Todas</button><button className={quick==='pendencias'?'active':''} onClick={()=>setQuick('pendencias')}><AlertTriangle size={15}/> Com pendências</button><button className={quick==='sem-gps'?'active':''} onClick={()=>setQuick('sem-gps')}><LocateFixed size={15}/> Sem GPS</button><button className={quick==='sem-equip'?'active':''} onClick={()=>setQuick('sem-equip')}><Cpu size={15}/> Sem equipamentos</button></div></section>
    <div className="resultsMeta"><b>{farms.length}</b> fazenda(s) encontrada(s)</div>
    {farms.length===0?<><Empty title="Nenhuma fazenda encontrada" text="Altere os filtros ou cadastre uma nova fazenda."/>{data.cloud&&data.fazendas.length===0&&<AccountDataNotice userId={data.userId}/>}</>:<div className="farmGrid">{farms.map(f=><FarmCard key={f.id} farm={f} data={data} onOpen={()=>onOpen(f.id)}/>)}</div>}
...
function EquipamentosFazenda({farm,data,openNew}){const [edit,setEdit]=useState(null); const equips=data.equipamentos.filter(e=>e.fazenda_id===farm.id); return <section className="panel"><div className="sectionTitle"><div><h2>Equipamentos instalados</h2><p className="sectionHint">A localização salva é protegida contra alterações acidentais.</p></div><button className="btn primary" onClick={openNew}><Plus size={17}/> Adicionar</button></div><div className="equipGrid">{equips.map(e=><div className="equipCard" key={e.id}><div className="equipIcon">{e.tipo?.includes('4102')?<RadioTower/>:<Cpu/>}</div><div><h3>{e.apelido||e.codigo_original||e.tipo}</h3><p>{e.tipo}</p><span>{e.local_nome||'Local não informado'} • {e.status}</span>{e.latitude&&<small><MapPin size={13}/> {Number(e.latitude).toFixed(6)}, {Number(e.longitude).toFixed(6)}</small>}{e.tipo?.includes('4102')&&<small><Target size={13}/> Raio estimado: {Number(e.raio_metros)||75} m</small>}</div><button className="iconBtn" title="Editar equipamento" onClick={()=>setEdit(e)}><Pencil size={17}/></button><button className="iconBtn danger" title="Excluir equipamento" onClick={()=>data.delEquipamento(e.id)}><Trash2 size={17}/></button></div>)}</div>{equips.length===0&&<Empty icon={Cpu} title="Nenhum equipamento" text="Cadastre VP8002, VP4102 ou outro equipamento da fazenda."/>}{edit&&<EquipModal farm={farm} equip={edit} onClose={()=>setEdit(null)} onSave={async(r)=>{await data.saveEquipamento(r);setEdit(null)}}/>}</section>}
...
function MapaFazenda({farm,data}){
  const all=data.equipamentos.filter(e=>e.fazenda_id===farm.id&&e.latitude&&e.longitude),[layer,setLayer]=useState('hibrido'),[focus,setFocus]=useState(null),[showBuffers,setShowBuffers]=useState(true),[showLabels,setShowLabels]=useState(true),[typeFilter,setTypeFilter]=useState('todos');
  ...
  return <section className="panel readOnlyMapPanel"><div className="sectionTitle"><div><h2>Mapa técnico operacional</h2><p>Todos os equipamentos cadastrados, com posição, identificação e cobertura estimada.</p></div><select value={layer} onChange={e=>setLayer(e.target.value)}><option value="hibrido">Híbrido</option><option value="mapa">Mapa com nomes</option><option value="satelite">Satélite</option></select></div><SearchMapControl onSelect={(lat,lng)=>setFocus([lat,lng])}/><div className="mapControls"><div className="filterChips"><button className={typeFilter==='todos'?'active':''} onClick={()=>setTypeFilter('todos')}>Todos ({all.length})</button><button className={typeFilter==='vp8002'?'active':''} onClick={()=>setTypeFilter('vp8002')}>VP8002</button><button className={typeFilter==='vp4102'?'active':''} onClick={()=>setTypeFilter('vp4102')}>VP4102</button><button className={typeFilter==='outros'?'active':''} onClick={()=>setTypeFilter('outros')}>Outros</button></div><label className="mapToggle"><input type="checkbox" checked={showBuffers} onChange={e=>setShowBuffers(e.target.checked)}/> Raios</label><label className="mapToggle"><input type="checkbox" checked={showLabels} onChange={e=>setShowLabels(e.target.checked)}/> Nomes</label></div><div className="mapLegend"><span><i className="legendProcessor"/> VP8002</span><span><i className="legendAntenna"/> VP4102</span><span><i className="legendOther"/> Outro equipamento</span><span><i className="legendBuffer"/> Cobertura estimada</span></div><div className="mapWrap technicalMap"><MapContainer center={center} zoom={all.length?17:15} className="bigMap"><HybridLayers layer={layer}/><RecenterMap position={focus}/>{farm.latitude&&farm.longitude&&<Marker position={[Number(farm.latitude),Number(farm.longitude)]} icon={farmMarkerIcon(farm)}><Popup><b>{farm.nome}</b><br/>Referência da fazenda</Popup></Marker>}{equips.map(e=><React.Fragment key={e.id}><Marker position={[Number(e.latitude),Number(e.longitude)]} icon={equipmentMarkerIcon({...e,apelido:showLabels?(e.apelido||e.codigo_original):''})}><Popup><b>{e.apelido||e.codigo_original||e.tipo}</b><br/>{e.tipo}<br/>Código: {e.codigo_original||'-'}<br/>Local: {e.local_nome||'-'}<br/>Status: {e.status}<br/>Coordenadas: {Number(e.latitude).toFixed(6)}, {Number(e.longitude).toFixed(6)}{e.tipo?.includes('4102')&&<><br/>Raio estimado: {Number(e.raio_metros)||75} m</>}</Popup></Marker>{showBuffers&&e.tipo?.includes('4102')&&<Circle center={[Number(e.latitude),Number(e.longitude)]} radius={Number(e.raio_metros)||75} pathOptions={{color:'#16a34a',fillColor:'#22c55e',fillOpacity:.16,weight:2}}/>}</React.Fragment>)}</MapContainer></div><div className="mapSummary"><div><b>{all.length}</b><span>equipamentos mapeados</span></div><div><b>{all.filter(e=>e.tipo?.includes('4102')).length}</b><span>antenas</span></div><div><b>{all.filter(e=>e.tipo?.includes('8002')).length}</b><span>processadores</span></div></div>{!all.length&&<Empty icon={MapIcon} title="Nenhum equipamento no mapa" text="Cadastre e posicione os equipamentos na aba Equipamentos."/>}</section>
}
...
function RelatorioFazenda({farm,data}){
  ...
  const exportTsv=()=>{const rows=[['RELATÓRIO TÉCNICO',farm.nome],['Central',farm.central||''],['Regional',farm.regional_nome||''],['Veterinário/Apoio',farm.veterinario_apoio||''],['Responsável',farm.responsavel||''],['Cidade',`${farm.cidade||''} / ${getFarmUF(farm)}`],['Status',farm.status||''],['Colares previstos',farm.qtd_colares_prevista||0],['Colares instalados',farm.qtd_colares_instalada||0],[],['EQUIPAMENTOS'],['Tipo','Código','Apelido','Local','Status','Raio (m)','Latitude','Longitude','Observações'],...equips.map(e=>[e.tipo,e.codigo_original,e.apelido,e.local_nome,e.status,e.raio_metros||'',e.latitude,e.longitude,e.observacoes||'']),[],['VISITAS'],['Data','Tipo','Resumo','Problemas','Solução','Pendências','Próxima ação'],...visits.map(v=>[v.data_visita,v.tipo,v.resumo,v.problemas,v.solucao,v.pendencias,v.proxima_acao])];download(`${farm.nome}-relatorio-tecnico.tsv`,rows.map(r=>r.join('\t')).join('\n'));notify('Arquivo TSV gerado.');};
  ...
  const reportCenter=mapped[0]?[Number(mapped[0].latitude),Number(mapped[0].longitude)]:farmLatLng(farm);
  return <section className="panel reportPanel"><div className="sectionTitle noPrint"><div><h2>Relatório técnico para coordenação</h2><p>Documento personalizado com mapa, equipamentos, coordenadas, visitas e pendências.</p></div><div className="headActions"><button className="btn light" onClick={share}><Share2 size={17}/> Compartilhar resumo</button><button className="btn primary" onClick={printReport}><Printer size={17}/> Imprimir / salvar PDF</button><button className="btn light" onClick={exportTsv}><FileDown size={17}/> Exportar dados</button></div></div><div className="reportOptions noPrint">{[['equip','Equipamentos'],['visits','Visitas'],['checks','Checklists'],['pend','Pendências']].map(([k,l])=><label className="optionCheck" key={k}><input type="checkbox" checked={opts[k]} onChange={()=>toggle(k)}/>{l}</label>)}</div><article className="printReport professionalReport"><header className="reportHeader"><Logo/><div><span>RELATÓRIO TÉCNICO DE INSTALAÇÃO E CAMPO</span><small>Emissão: {new Date().toLocaleString('pt-BR')}</small></div></header><section className="reportCover"><div><span className="reportTag">CONTROLTECH ASSIST</span><h1>{farm.nome}</h1><p>{farm.cidade||'Cidade não informada'} / {getFarmUF(farm)||'-'}</p></div><div className="reportStatus"><b>{farm.status||'Não informado'}</b><span>Situação da operação</span></div></section><div className="reportMeta"><div><b>Central atendida</b><span>{farm.central||'-'}</span></div><div><b>Regional</b><span>{farm.regional_nome||'-'}</span></div><div><b>Veterinário/Apoio</b><span>{farm.veterinario_apoio||'-'}</span></div><div><b>Responsável da fazenda</b><span>{farm.responsavel||'-'}</span></div><div><b>Telefone</b><span>{farm.telefone||'-'}</span></div><div><b>Endereço</b><span>{farm.endereco||'-'}</span></div></div><div className="reportMetrics"><div><b>{num(farm.qtd_colares_prevista)}</b><span>Colares previstos</span></div><div><b>{num(farm.qtd_colares_instalada)}</b><span>Colares instalados</span></div><div><b>{equips.length}</b><span>Equipamentos</span></div><div><b>{visits.length}</b><span>Visitas registradas</span></div></div><section className="reportSection"><div className="reportSectionHead"><h2>Mapa técnico da instalação</h2><p>Equipamentos cadastrados, apelidos, coordenadas e raios estimados das antenas.</p></div><div className="reportMap"><MapContainer center={reportCenter} zoom={mapped.length?16:13} className="bigMap" zoomControl={false} dragging={false} scrollWheelZoom={false} doubleClickZoom={false}><HybridLayers layer="hibrido"/>{mapped.map((e,i)=><React.Fragment key={e.id}><Marker position={[Number(e.latitude),Number(e.longitude)]} icon={equipmentMarkerIcon({...e,apelido:`${i+1} · ${e.apelido||e.codigo_original||e.tipo}`})}><Popup>{e.apelido||e.codigo_original}</Popup></Marker>{e.tipo?.includes('4102')&&<Circle center={[Number(e.latitude),Number(e.longitude)]} radius={Number(e.raio_metros)||75} pathOptions={{color:'#22c55e',fillColor:'#22c55e',fillOpacity:.14}}/>}</React.Fragment>)}</MapContainer></div><div className="reportMapKey">{mapped.map((e,i)=><div key={e.id}><b>{i+1}</b><span>{e.apelido||e.codigo_original||e.tipo}</span><small>{Number(e.latitude).toFixed(6)}, {Number(e.longitude).toFixed(6)}{e.tipo?.includes('4102')?` • raio ${Number(e.raio_metros)||75} m`:''}</small></div>)}</div></section><section className="reportSection"><h2>Resumo executivo</h2><p>{farm.observacoes||'Instalação registrada no ControlTech Assist. Consulte equipamentos e visitas abaixo para detalhes operacionais.'}</p></section>{opts.equip&&<section className="reportSection"><h2>Relação de equipamentos</h2>{equips.length?<div className="reportEquipmentList">{equips.map((e,i)=><article key={e.id}><div className="reportEquipIndex">{i+1}</div><div><h3>{e.apelido||e.codigo_original||e.tipo}</h3><p>{e.tipo} • {e.status}</p><dl><div><dt>Código</dt><dd>{e.codigo_original||'-'}</dd></div><div><dt>Local</dt><dd>{e.local_nome||'-'}</dd></div><div><dt>Coordenadas</dt><dd>{e.latitude&&e.longitude?`${Number(e.latitude).toFixed(6)}, ${Number(e.longitude).toFixed(6)}`:'-'}</dd></div><div><dt>Raio estimado</dt><dd>{e.tipo?.includes('4102')?`${Number(e.raio_metros)||75} m`:'Não se aplica'}</dd></div></dl>{e.observacoes&&<small>{e.observacoes}</small>}</div></article>)}</div>:<p>Nenhum equipamento registrado.</p>}</section>}{opts.visits&&<section className="reportSection"><h2>Histórico de visitas</h2>{visits.length?visits.map(v=><div className="reportVisit" key={v.id}><b>{brDate(v.data_visita)} — {v.tipo}</b><p>{v.resumo||'Sem resumo.'}</p>{v.problemas&&<p><strong>Problemas:</strong> {v.problemas}</p>}{v.solucao&&<p><strong>Solução:</strong> {v.solucao}</p>}{opts.pend&&v.pendencias&&<p className="reportPending"><strong>Pendências:</strong> {v.pendencias}</p>}{v.proxima_acao&&<p><strong>Próxima ação:</strong> {v.proxima_acao}</p>}</div>):<p>Nenhuma visita registrada.</p>}</section>}{opts.checks&&<section className="reportSection"><h2>Checklists</h2><p>{checks.length} checklist(s) salvo(s) para esta fazenda.</p></section>}<section className="reportSignature"><div><span>Responsável técnico</span></div><div><span>Coordenação / Cliente</span></div></section><footer className="reportFooter"><span>ControlTech Assist • Documento técnico de campo</span><span>Versão {APP_VERSION}</span></footer></article></section>}
