...
import {
  ...
  ClipboardPenLine, LifeBuoy, FileDown, Antenna, Gauge, ScanLine
} from 'lucide-react';
import './styles.css';
...
function useData(user){
  ...
  async function upsert(table, setter, key, row){
    ...
    if(cloud){ const { error } = await supabase.from(table).upsert(clean); if(error) alert(error.message); }
  ...
  async function remove(table, setter, key, id){
    ...
    if(cloud){ const { error } = await supabase.from(table).delete().eq('id',id); if(error) alert(error.message); }
    setter(prev => { const next=prev.filter(x=>x.id!==id); if(!cloud) saveLocal(key,next); return next; });
  }
  return {
    cloud, loading, fazendas, equipamentos, visitas, checklists, diagnosticos,
...
function App(){
  const [user,setUser]=useState(null); const [authLoading,setAuthLoading]=useState(Boolean(supabase)); const [view,setView]=useState('fazendas'); const [selectedFarmId,setSelectedFarmId]=useState(null);
  const data=useData(user); const selectedFarm=data.fazendas.find(f=>f.id===selectedFarmId);
  useEffect(()=>{ if(!supabase) return; supabase.auth.getSession().then(({data})=>{setUser(data.session?.user||null); setAuthLoading(false)}); const {data:sub}=supabase.auth.onAuthStateChange((_e,session)=>setUser(session?.user||null)); return ()=>sub.subscription.unsubscribe(); },[]);
  if(authLoading) return <Splash/>; if(supabase && !user) return <Login/>;
  const goFarm = id => { setSelectedFarmId(id); setView('fazenda'); };
  const setMainView = v => { setView(v); if(v!=='fazenda') setSelectedFarmId(null); };
  return <div className="app"><Sidebar view={view} setView={setMainView} user={user} cloud={data.cloud}/><main className="main">
...
function Sidebar({view,setView,user,cloud}){const items=[['fazendas',MapPinned,'Fazendas'],['instalacao',Route,'Instalação'],['diagnostico',Stethoscope,'Diagnóstico'],['guia',BookOpen,'Guia'],['relatorios',BarChart3,'Relatórios']];return <aside className="sidebar"><Logo/><nav>{items.map(([id,Icon,label])=><button key={id} className={view===id?'active':''} onClick={()=>setView(id)}><Icon size={20}/>{label}</button>)}</nav><div className="sideFoot"><span className={cloud?'cloud on':'cloud'}><Database size={15}/>{cloud?'Supabase ativo':'Modo local'}</span>{user&&<button className="logout" onClick={()=>supabase.auth.signOut()}><LogOut size={16}/> Sair</button>}</div></aside>}
...
function Empty({icon:Icon,title,text}){return <div className="empty"><Icon size={42}/><h3>{title}</h3><p>{text}</p></div>}

function Fazendas({data,onOpen}){
  const [q,setQ]=useState(''),[modal,setModal]=useState(false); const farms=data.fazendas.filter(f=>[f.nome,f.cidade,f.responsavel].join(' ').toLowerCase().includes(q.toLowerCase()));
  const emAndamento=data.fazendas.filter(f=>f.status==='Em andamento').length; const finalizadas=data.fazendas.filter(f=>f.status==='Finalizada'||f.status==='Instalação concluída').length;
  return <div><PageHead eyebrow="Controle de visitas" title="Minhas fazendas" desc="Abra uma fazenda para gerenciar equipamentos, colares, checklists, mapa e visitas."><button className="btn primary" onClick={()=>setModal(true)}><Plus size={18}/> Nova fazenda</button></PageHead>
    <div className="statsGrid"><Stat icon={MapPinned} label="fazendas" value={data.fazendas.length}/><Stat icon={Wrench} label="em andamento" value={emAndamento} tone="green"/><Stat icon={BadgeCheck} label="finalizadas" value={finalizadas}/><Stat icon={Hash} label="colares previstos" value={data.fazendas.reduce((a,f)=>a+num(f.qtd_colares_prevista),0)}/></div>
    <div className="toolbar"><div className="search"><Search size={18}/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar fazenda, cidade ou responsável..."/></div></div>
    <div className="farmGrid">{farms.map(f=><FarmCard key={f.id} farm={f} data={data} onOpen={()=>onOpen(f.id)}/>)}</div>{farms.length===0&&<Empty icon={MapPinned} title="Nenhuma fazenda encontrada" text="Cadastre a primeira fazenda para iniciar o controle técnico."/>}
    {modal&&<FazendaModal onClose={()=>setModal(false)} onSave={async(r)=>{await data.saveFazenda(r);setModal(false)}}/>}</div>
}
function FarmCard({farm,data,onOpen}){const eq=data.equipamentos.filter(e=>e.fazenda_id===farm.id).length; const visits=data.visitas.filter(v=>v.fazenda_id===farm.id); const pct=num(farm.qtd_colares_prevista)?Math.round(num(farm.qtd_colares_instalada)/num(farm.qtd_colares_prevista)*100):0; return <article className="farmCard" onClick={onOpen}><div className="cardTop"><div className="badgeIcon"><Building2 size={20}/></div><span className={`status ${farm.status?.includes('pend')?'warn':farm.status?.includes('Final')||farm.status?.includes('conclu')?'ok':''}`}>{farm.status||'Não iniciada'}</span></div><h3>{farm.nome}</h3><p><MapPin size={15}/>{farm.cidade||'Cidade não informada'}</p><p><User size={15}/>{farm.responsavel||'Responsável não informado'}</p><div className="progress"><span style={{width:`${Math.min(pct,100)}%`}}/></div><div className="miniStats"><span><b>{num(farm.qtd_colares_instalada)}</b> instalados</span><span><b>{num(farm.qtd_colares_prevista)}</b> previstos</span><span><b>{eq}</b> equips.</span></div><footer>Última visita: {visits[0]?brDate(visits[0].data_visita):'sem visita'}<ChevronLeft className="rotate" size={17}/></footer></article>}
function FazendaModal({farm={},onClose,onSave}){const [form,setForm]=useState({id:farm.id||uid(),nome:farm.nome||'',responsavel:farm.responsavel||'',telefone:farm.telefone||'',cidade:farm.cidade||'',endereco:farm.endereco||'',qtd_colares_prevista:farm.qtd_colares_prevista||'',qtd_colares_instalada:farm.qtd_colares_instalada||'',status:farm.status||'Não iniciada',observacoes:farm.observacoes||'',created_at:farm.created_at||nowISO()}); const set=(k,v)=>setForm({...form,[k]:v}); return <Modal title={farm.id?'Editar fazenda':'Nova fazenda'} onClose={onClose}><form className="form modern" onSubmit={e=>{e.preventDefault();onSave({...form,qtd_colares_prevista:num(form.qtd_colares_prevista),qtd_colares_instalada:num(form.qtd_colares_instalada)})}}><Field label="Nome da fazenda *" icon={Building2}><input value={form.nome} onChange={e=>set('nome',e.target.value)} required placeholder="Ex: Fazenda Santa Maria"/></Field><div className="grid2"><Field label="Responsável" icon={User}><input value={form.responsavel} onChange={e=>set('responsavel',e.target.value)} placeholder="Nome"/></Field><Field label="Telefone" icon={Phone}><input value={form.telefone} onChange={e=>set('telefone',e.target.value)} placeholder="(00) 00000-0000"/></Field></div><div className="grid2"><Field label="Cidade" icon={MapPin}><input value={form.cidade} onChange={e=>set('cidade',e.target.value)} placeholder="Cidade/UF"/></Field><Field label="Status" icon={BadgeCheck}><select value={form.status} onChange={e=>set('status',e.target.value)}>{FARM_STATUS.map(s=><option key={s}>{s}</option>)}</select></Field></div><Field label="Endereço"><input value={form.endereco} onChange={e=>set('endereco',e.target.value)} placeholder="Endereço ou referência"/></Field><div className="grid2"><Field label="Colares previstos" icon={Hash}><input type="number" value={form.qtd_colares_prevista} onChange={e=>set('qtd_colares_prevista',e.target.value)}/></Field><Field label="Colares instalados" icon={CheckCircle2}><input type="number" value={form.qtd_colares_instalada} onChange={e=>set('qtd_colares_instalada',e.target.value)}/></Field></div><Field label="Observações"><textarea value={form.observacoes} onChange={e=>set('observacoes',e.target.value)} placeholder="Informações importantes da fazenda"/></Field><button className="btn primary full"><Save size={18}/> Salvar</button></form></Modal>}
...
function MapaFazenda({farm,data}){const equips=data.equipamentos.filter(e=>e.fazenda_id===farm.id&&e.latitude&&e.longitude); const center=equips[0]?[Number(equips[0].latitude),Number(equips[0].longitude)]:[-19.7483,-47.9319]; return <section className="panel"><div className="sectionTitle"><h2>Mapa técnico da instalação</h2><span className="pill"><Satellite size={15}/> Satélite</span></div><div className="mapWrap"><MapContainer center={center} zoom={equips.length?17:13} className="bigMap"><TileLayer url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}" attribution="Tiles Esri"/>{equips.map(e=><React.Fragment key={e.id}><Marker position={[Number(e.latitude),Number(e.longitude)]}><Popup><b>{e.apelido||e.codigo_original}</b><br/>{e.tipo}<br/>{e.local_nome}<br/>{e.status}</Popup></Marker>{e.tipo?.includes('4102')&&<Circle center={[Number(e.latitude),Number(e.longitude)]} radius={e.observacoes?.toLowerCase().includes('extern')?250:75} pathOptions={{color:'#22c55e',fillColor:'#22c55e',fillOpacity:.12}}/>}</React.Fragment>)}</MapContainer></div><p className="sourceText">Raio visual estimado: VP4102 interna ≈ 75 m; externa ≈ 250 m com visada livre. Use como apoio, não como medição exata.</p><div className="list compact">{equips.map(e=><div className="listItem" key={e.id}>{e.tipo?.includes('4102')?<RadioTower/>:<Cpu/>}<div><b>{e.apelido||e.codigo_original}</b><span>{e.local_nome||'-'} • {Number(e.latitude).toFixed(6)}, {Number(e.longitude).toFixed(6)}</span></div></div>)}</div></section>}
...
function Relatorios({data,onOpen}){const exportGeral=()=>{const rows=[['Fazenda','Cidade','Responsável','Status','Colares previstos','Colares instalados','Equipamentos','Visitas'],...data.fazendas.map(f=>[f.nome,f.cidade,f.responsavel,f.status,f.qtd_colares_prevista,f.qtd_colares_instalada,data.equipamentos.filter(e=>e.fazenda_id===f.id).length,data.visitas.filter(v=>v.fazenda_id===f.id).length])]; download('relatorio-geral-fazendas.tsv', rows.map(r=>r.join('\t')).join('\n'));}; return <div><PageHead eyebrow="Relatórios" title="Relatório geral" desc="Visão geral das fazendas visitadas, colares, equipamentos e visitas."><button className="btn primary" onClick={exportGeral}><Download size={18}/> Exportar geral</button></PageHead><div className="tableWrap"><table><thead><tr><th>Fazenda</th><th>Status</th><th>Colares</th><th>Equip.</th><th>Visitas</th><th></th></tr></thead><tbody>{data.fazendas.map(f=><tr key={f.id}><td><b>{f.nome}</b><span>{f.cidade||'-'}</span></td><td>{f.status}</td><td>{num(f.qtd_colares_instalada)} / {num(f.qtd_colares_prevista)}</td><td>{data.equipamentos.filter(e=>e.fazenda_id===f.id).length}</td><td>{data.visitas.filter(v=>v.fazenda_id===f.id).length}</td><td><button className="btn light" onClick={()=>onOpen(f.id)}>Abrir</button></td></tr>)}</tbody></table></div></div>}
