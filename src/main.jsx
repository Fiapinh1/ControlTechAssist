...
const ROUTINE_ASSIST_URL = import.meta.env.VITE_ROUTINE_ASSIST_URL || 'http://127.0.0.1:5173/';
...
function BottomNav({view,setView,user,localMode,onLogout,isAdmin=false}){const items=[['fazendas',Home,'Início'],['produtividade',BarChart3,'Prod.'],['suporte',LifeBuoy,'Suporte'],['relatorios',FileText,'Rel.'],isAdmin&&['gerencial',ShieldCheck,'Adm.']].filter(Boolean);const columns=items.length+1+(user||localMode?1:0);return <nav className="bottomNav" style={{gridTemplateColumns:`repeat(${columns},1fr)`}}>{items.map(([id,Icon,label])=><button key={id} className={view===id?'active':''} onClick={()=>setView(id)}><Icon size={20}/><span>{label}</span></button>)}<a className="appExternalNav" href={ROUTINE_ASSIST_URL} target="_blank" rel="noreferrer" aria-label="Abrir Routine Assist"><img src="/routine-assist-logo.png" alt=""/><span>Routine</span></a>{(user||localMode)&&<button className="mobileLogout" onClick={onLogout}><LogOut size={20}/><span>Sair</span></button>}</nav>}
