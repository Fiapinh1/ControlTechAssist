...
function ReportPreview({farm,equips,visits,checks,mapped,evidencias=[]}){
  ...
  return <article className={`printReport professionalReport reportPreview theme-${reportBrand.key}`}>
    ...
    <section className="reportSection"><h2>Dados da fazenda</h2><div className="reportInfoGrid"><div><span>Central</span><b>{centralOfficialName(farm.central)||'-'}</b></div><div><span>Regional</span><b>{farm.regional_nome||'-'}</b></div><div><span>Responsavel</span><b>{farm.responsavel||'-'}</b></div><div><span>Telefone</span><b>{farm.telefone||'-'}</b></div><div><span>Veterinario/Apoio</span><b>{farm.veterinario_apoio||'-'}</b></div><div><span>Endereco</span><b>{farm.endereco||'-'}</b></div><div><span>Inicio do servico</span><b>{brDateTime(farm.servico_inicio_em)}</b></div><div><span>Fim do servico</span><b>{brDateTime(farm.servico_fim_em)}</b></div><div><span>Responsavel tecnico</span><b>{farm.servico_responsavel||'-'}</b></div></div></section>
