...
function RelatorioFazenda({farm,data}){
  ...
  const printReport=()=>{
    ...
    const mapKey=points.map(p=>`<div><b class="${p.kind}">${p.kind==='farm'?'F':p.index}</b><span>${safe(p.label)}</span><small>${p.lat.toFixed(6)}, ${p.lng.toFixed(6)}</small></div>`).join('');
    ...
    const win=window.open('','_blank');if(!win){notify('Permita pop-ups para gerar o relatório.','error');return;}win.document.write(html);win.document.close();
  };
...
function Produtividade({data,onOpen}){
  ...
  const farms=concluded.filter(matchFarm).sort((a,b)=>new Date(b.servico_fim_em)-new Date(a.servico_fim_em));
  const inProgress=data.fazendas.filter(f=>f.servico_inicio_em&&!f.servico_fim_em);
  const totalHours=farms.reduce((a,f)=>a+serviceHours(f),0);
  const totalCollars=farms.reduce((a,f)=>a+num(f.qtd_colares_instalada||f.qtd_colares_prevista),0);
  const avgHours=farms.length?totalHours/farms.length:0;
  const avgLabel=avgHours?(avgHours<24?`${avgHours.toFixed(1)} h`:`${(avgHours/24).toFixed(1)} dias`):'-';
  const collarsPerDay=totalHours?totalCollars/(totalHours/24):0;
  ...
  const monthly=monthNames.map((label,i)=>({label,value:monthlyBase.filter(f=>new Date(f.servico_fim_em).getMonth()===i).length}));
  const buckets=[['0-100',0,100],['101-250',101,250],['251-500',251,500],['501+',501,Infinity]].map(([label,min,max])=>{
    const list=farms.filter(f=>{const collars=num(f.qtd_colares_instalada||f.qtd_colares_prevista);return collars>=min&&collars<=max;});
    return {label,value:list.length?list.reduce((a,f)=>a+serviceHours(f),0)/list.length/24:0};
  });
  const exportData=()=>{const rows=[['Fazenda','Central','Cidade','Responsavel produtividade','Inicio','Fim','Duracao horas','Colares instalados','Colares previstos'],...farms.map(f=>[f.nome,f.central,f.cidade,f.servico_responsavel||f.regional_nome||f.responsavel,brDateTime(f.servico_inicio_em),brDateTime(f.servico_fim_em),serviceHours(f).toFixed(2),num(f.qtd_colares_instalada),num(f.qtd_colares_prevista)])];download('produtividade-fazendas.tsv',rows.map(r=>r.join(String.fromCharCode(9))).join(String.fromCharCode(10)));};
  return <div><PageHead eyebrow="Produtividade" title="Controle de produtividade" desc="Acompanhe tempo de instalacao por fazenda, periodo, central, equipe e quantidade de colares."><button className="btn light" onClick={exportData}><Download size={18}/> Exportar analise</button></PageHead><div className="statsGrid"><Stat icon={Building2} label="fazendas concluidas" value={farms.length}/><Stat icon={Clock} label="tempo medio" value={avgLabel}/><Stat icon={Hash} label="colares/dia" value={collarsPerDay?collarsPerDay.toFixed(1):'-'} tone="green"/><Stat icon={PlayCircle} label="em andamento" value={inProgress.length}/></div><section className="panel productivityFilters"><div className="sectionTitle"><div><h2><Filter size={20}/> Filtros</h2><p className="sectionHint">Use o periodo de conclusao para fechar produtividade mensal ou anual.</p></div></div><div className="grid4"><Field label="Ano"><select value={year} onChange={e=>setYear(e.target.value)}><option>Todos</option>{years.map(y=><option key={y}>{y}</option>)}</select></Field><Field label="Mes"><select value={month} onChange={e=>setMonth(e.target.value)}><option>Todos</option>{monthNames.map((m,i)=><option key={m} value={i+1}>{m}</option>)}</select></Field><Field label="Central"><select value={central} onChange={e=>setCentral(e.target.value)}><option>Todas</option>{CENTRAIS.map(c=><option key={c}>{c}</option>)}</select></Field><Field label="Responsavel"><select value={resp} onChange={e=>setResp(e.target.value)}><option>Todos</option>{responsaveis.map(r=><option key={r}>{r}</option>)}</select></Field></div></section><div className="productivityGrid"><section className="panel prodChart"><div className="sectionTitle"><div><h2><BarChart3 size={20}/> Instalacoes por mes</h2><p className="sectionHint">Quantidade de fazendas finalizadas no ano filtrado.</p></div></div><ProductivityBars rows={monthly}/></section><section className="panel prodChart"><div className="sectionTitle"><div><h2><Gauge size={20}/> Tempo medio por colares</h2><p className="sectionHint">Media em dias por faixa de colares instalados ou previstos.</p></div></div><ProductivityBars rows={buckets} suffix=" d" decimals={1}/></section></div><section className="panel"><div className="sectionTitle"><div><h2><ClipboardList size={20}/> Fazendas analisadas</h2><p className="sectionHint">Apenas fazendas com inicio e fim registrados entram nos calculos.</p></div><span className="pill">{totalCollars} colares</span></div>{farms.length?<div className="prodTable"><table><thead><tr><th>Fazenda</th><th>Periodo</th><th>Duracao</th><th>Colares</th><th>Responsavel</th><th></th></tr></thead><tbody>{farms.map(f=><tr key={f.id}><td><b>{f.nome}</b><span>{f.cidade||'-'} • {f.central||'-'}</span></td><td>{brDate(f.servico_inicio_em)}<span>ate {brDate(f.servico_fim_em)}</span></td><td>{serviceDurationLabel(f)}</td><td>{num(f.qtd_colares_instalada)||num(f.qtd_colares_prevista)}</td><td>{f.servico_responsavel||f.regional_nome||f.responsavel||'-'}</td><td><button className="btn light" onClick={()=>onOpen(f.id)}>Abrir</button></td></tr>)}</tbody></table></div>:<Empty icon={Clock} title="Sem dados de produtividade" text="Abra uma fazenda e registre inicio e fim do servico para gerar os graficos."/>}</section>{inProgress.length>0&&<section className="panel"><div className="sectionTitle"><div><h2><PlayCircle size={20}/> Servicos em andamento</h2><p className="sectionHint">Fazendas iniciadas e ainda sem data de fim.</p></div></div><div className="serviceOpenList">{inProgress.map(f=><button key={f.id} onClick={()=>onOpen(f.id)}><Clock size={18}/><span><b>{f.nome}</b><small>{brDateTime(f.servico_inicio_em)} • {serviceDurationLabel(f)}</small></span></button>)}</div></section>}</div>
}
